...
class MixEncoder(nn.Module):
    def __init__(
        self, input_dim=3, output_dim=128, stride=8, norm_fn="batch", dropout=0.0, model_name='dino_vits8'
    ):
        super(MixEncoder, self).__init__()
        self.stride = stride
        self.model_name = model_name
        self.dino = torch.hub.load("facebookresearch/dino:main", model_name)
        # self.dino = self.dino.eval()
        self.dino.requires_grad_(False)

        self.patch_size =  8 if "8" in self.model_name else 14
        self.cnn_backbone = BasicEncoder(input_dim, output_dim, stride, norm_fn, dropout)
    ...
    def forward(self, x):
        ...
        h_stride = H // self.stride
        w_stride = W // self.stride
        ...
        global_token = repeat(global_token, "b c -> b c h w", h=h_stride, w=w_stride)
        ...
        local_tokens = F.interpolate(local_tokens, size=[h_stride, w_stride], mode='bilinear')
    
        cnn_features = self.cnn_backbone(x)
        return cnn_features + local_tokens + global_token
