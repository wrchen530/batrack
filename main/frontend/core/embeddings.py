...
def get_nd_embedding(coords, C, cat_coords=True):
    B, N, D = coords.shape


    div_term = (
        torch.arange(0, C, 2, device=coords.device, dtype=torch.float32) * (1000.0 / C)
    ).reshape(1, 1, int(C / 2))

    pe_list = []
    for i in range(D):
        pe_i = torch.zeros(B, N, C, device=coords.device, dtype=torch.float32)
        pe_i[:, :, 0::2] = torch.sin(coords[...,i:(i+1)] * div_term)
        pe_i[:, :, 1::2] = torch.cos(coords[...,i:(i+1)] * div_term)

        pe_list.append(pe_i)

    pe = torch.cat(pe_list, dim=2)
    if cat_coords:
        pe = torch.cat([pe, coords], dim=2)  # B, N, C*3+3
    return pe
