...
def test_all(result_dir, grid_size=4, scenes=None, align_depth=True, loss_weight_dict={}, depth_min=1e-2, depth_max=1e2, niter=200, scaling='median', fixed_pose=False, fixed_K=False):
    ...
    for scene in scenes:
        ...
        result_path = os.path.join(result_dir, scene_name, 'results.pkl')
        if not os.path.exists(result_path):
            print(f"skip {scene_name}", result_path)
            continue
        ...
        global_alignment_loop(refine_net, lr=1e-2, niter=niter, schedule='cosine', lr_min=1e-6, fixed_pose=True, fixed_K=True)
...
if __name__ == '__main__':
    ...
    test_all(
        result_dir, grid_size=args.grid_size, scenes=scenes, loss_weight_dict=loss_weight_dict, niter=args.niter,
