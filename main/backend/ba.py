...
def BA(poses, patches, intrinsics, targets, weights, lmbda, ii, jj, kk, bounds, ep=100.0, PRINT=False, fixedp=1, structure_only=False, loss='trivial'):
    ...
    p = coords.shape[3]
    # compute residual
    r = targets - coords[...,p//2,p//2,:]   # B, N_edge, 2
...
def BA_rgbd_droid(poses, patches, patches_monodisp, intrinsics, targets_2d, targets_disp, weights, lmbda, ii, jj, kk, bounds, ep=100.0, PRINT=False, fixedp=1, structure_only=False, loss='trivial', alpha=0.5):
    b = 1
    n = max(ii.max().item(), jj.max().item()) + 1
    ...
    coords, v, (Ji, Jj, Jz) = \
        pops.transform(poses, patches, intrinsics, ii, jj, kk, jacobian=True, depth=False)

    # coords = coords_3d[...,:2]
    # coords_disp = coords_3d[...,2:]
    p = coords.shape[3]
    # compute residual
    r = targets_2d - coords[...,p//2,p//2,:]   # B, N_edge, 2
    ...
    # alpha = 0.5 # Weighting factor for depth sensor measurements

    m = (patches_monodisp[:, kx,:,None] > 1e-2).float()  # Shape: (B, m, 1, 1)
    ...
    C_adjusted = C + m * alpha
    C_adjusted = C_adjusted + lmbda
 
    disps_kx = disps[:, kx, 0, 0, None, None]  # Shape: (B, m, 1, 1)
    disps_sens_kx = patches_monodisp[:, kx, :, None]  # Shape: (B, m, 1, 1)
    w_adjusted = w - m * alpha * (disps_kx - disps_sens_kx) 
    ...
    if structure_only or n == 0:
        dZ = (Q * w_adjusted).view(b, -1, 1, 1)

    else:
        # use schue elimination to solve the camera update first
        S = B - block_matmul(EQ, E.permute(0,2,1,4,3))
        y = v - block_matmul(EQ, w_adjusted.unsqueeze(dim=2))
        dX = block_solve(S, y, ep=ep, lm=1e-4)
        if torch.isnan(dX).any():
            dX = block_solve(S, y, ep=ep, lm=1e-3)

        # solve the depth update with camera update
        dZ = Q * (w_adjusted - block_matmul(E.permute(0,2,1,4,3), dX).squeeze(dim=-1))
        dX = dX.view(b, -1, 6)
        dZ = dZ.view(b, -1, 1, 1)
